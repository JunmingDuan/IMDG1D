...
switch n;
case 20;
  numer1 = load(['example2_Nx20_K',num2str(K),'.dat']);
  x1 = numer1(:,1); y1 = numer1(:,3);
  plot(exact(:,1), exact(:,2), '-k', x1, y1, 'ro');
  ex1 = f(x1);
  err(1,1) = sqrt(sum((ex1-y1).^2.*numer1(:,2)*2*pi/20/2));
  err(1,2) = sum(abs(ex1-y1).*numer1(:,2)*2*pi/20/2);
  ...
  disp(err);
case 40;
  numer2 = load(['example2_Nx40_K',num2str(K),'.dat']);
  x2 = numer2(:,1); y2 = numer2(:,3);
  plot(exact(:,1), exact(:,2), '-k', x2, y2, 'ro');
  ex2 = f(x2);
  err(2,1) = sqrt(sum((ex2-y2).^2.*numer2(:,2)*2*pi/40/2));
  err(2,2) = sum(abs(ex2-y2).*numer2(:,2)*2*pi/40/2);
  ...
  disp(err);
case 80;
  numer3 = load(['example2_Nx80_K',num2str(K),'.dat']);
  x3 = numer3(:,1); y3 = numer3(:,3);
  plot(exact(:,1), exact(:,2), '-k', x3, y3, 'ro');
  ex3 = f(x3);
  err(3,1) = sqrt(sum((ex3-y3).^2.*numer3(:,2)*2*pi/80/2));
  err(3,2) = sum(abs(ex3-y3).*numer3(:,2)*2*pi/80/2);
  ...
  disp(err);
case 160;
  numer4 = load(['example2_Nx160_K',num2str(K),'.dat']);
  x4 = numer4(:,1); y4 = numer4(:,3);
  plot(exact(:,1), exact(:,2), '-k', x4, y4, 'ro');
  ex4 = f(x4);
  err(4,1) = sqrt(sum((ex4-y4).^2.*numer4(:,2)*2*pi/160/2));
  err(4,2) = sum(abs(ex4-y4).*numer4(:,2)*2*pi/160/2);
  ...
  disp(err);
case 320;
  numer5 = load(['example2_Nx320_K',num2str(K),'.dat']);
  x5 = numer5(:,1); y5 = numer5(:,3);
  plot(exact(:,1), exact(:,2), '-k', x5, y5, 'ro');
  ex5 = f(x5);
  err(5,1) = sqrt(sum((ex5-y5).^2.*numer5(:,2)*2*pi/320/2));
  err(5,2) = sum(abs(ex5-y5).*numer5(:,2)*2*pi/320/2);
